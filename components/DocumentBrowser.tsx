    ...
    <div className="flex flex-col h-[650px] lg:h-[700px]">
      ...
      <div className="mb-4">
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-xl font-bold text-santa-teal">Vector DB Browser</h2>
          <div className="flex gap-2">
            ...
            <button
              ...
              className="px-4 py-2 text-sm font-medium text-santa-teal hover:bg-santa-teal hover:text-white rounded-lg transition-colors"
            >
              <span className="flex items-center gap-2">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
        ...
        <input
          ...
          className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-santa-teal"
        />
      </div>
      ...
      <div className="flex-1 flex gap-4 min-h-0">
        ...
        <div className="w-80 flex-shrink-0 bg-white border border-gray-200 rounded-xl p-4 overflow-y-auto">
          <h3 className="font-semibold text-santa-teal mb-3">
            Documents ({filteredDocuments.length})
          </h3>
          {loading ? (
            <div className="text-center py-8">
              <div className="inline-block animate-spin rounded-full h-8 w-8 border-b-2 border-santa-teal"></div>
                ...
                <div
                  ...
                  className={`p-3 rounded-lg border cursor-pointer transition-colors ${
                    selectedDoc?.fileId === doc.fileId
                      ? 'bg-santa-teal border-santa-teal text-white'
                      : 'bg-santa-cream border-santa-teal/20 hover:bg-santa-orange-light/50'
                  }`}
                >
                  <p className={`text-sm font-medium truncate ${selectedDoc?.fileId === doc.fileId ? 'text-white' : 'text-santa-teal'}`} title={doc.title || doc.name || 'Untitled Document'}>
            ...
            <>
              <div className="mb-4 pb-4 border-b border-gray-200">
                <div className="flex items-center justify-between mb-2">
                  <h3 className="text-lg font-semibold text-santa-teal">{selectedDoc.title || selectedDoc.name || 'Untitled Document'}</h3>
                  {chunks.length > 0 && (
