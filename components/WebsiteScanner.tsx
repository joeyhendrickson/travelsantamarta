  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      <div>
          ...
          <div className="flex items-center gap-3">
            {onBack && (
              <button
                type="button"
                onClick={onBack}
                className="p-2 rounded-lg hover:bg-santa-orange-light/50 transition-colors"
        ...
        <p className="text-santa-teal/80 text-lg leading-relaxed">
        ...
        <div className="flex items-center justify-between bg-white border-2 border-gray-200 rounded-xl p-4">
          ...
          <div className="flex gap-3">
            <button
              ...
              className="px-6 py-2 bg-santa-teal text-white rounded-lg font-semibold hover:bg-santa-tealLight disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
                    ...
                    {scanned.analysis && (
                      <div className="mt-4 space-y-3">
                        ...
                        <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                          ...
                          <p className="text-sm text-santa-teal/80">{scanned.analysis.summary}</p>
                        </div>
                          ...
                          <div>
                            ...
                            <ul className="list-disc list-inside space-y-1">
                              {scanned.analysis.risks.map((risk, riskIndex) => (
                                <li key={riskIndex} className="text-sm text-santa-teal/80">
                                  {risk}
                                </li>
                              ))}
                            </ul>
                            ...
                            <div className="space-y-2">
                              {scanned.analysis.issues.map((issue, issueIndex) => (
                                ...
                                >
                                  ...
                                  <p className="text-xs text-santa-teal/80 mb-1">{issue.description}</p>
                                  {issue.reasoning && (
                        ...
                        {scanned.analysis.currentRelevance && (
                          <div className="mt-3 p-4 bg-green-50 border border-green-200 rounded-lg">
                            ...
                            {scanned.analysis.currentRelevance.accurateContent && scanned.analysis.currentRelevance.accurateContent.length > 0 && (
                              <div className="mt-2">
                                <p className="text-xs font-semibold text-santa-teal/80 mb-1">Still Accurate:</p>
                                <ul className="list-disc list-inside text-xs text-santa-teal/80">
                            ...
                            {scanned.analysis.currentRelevance.needsUpdating && scanned.analysis.currentRelevance.needsUpdating.length > 0 && (
                              <div className="mt-2">
                                <p className="text-xs font-semibold text-santa-teal/80 mb-1">Needs Updating:</p>
                                <ul className="list-disc list-inside text-xs text-santa-teal/80">
                        ...
                        {scanned.analysis.messagingStrategy && (
                          <div className="mt-3 p-4 bg-indigo-50 border border-indigo-200 rounded-lg">
                            ...
                            {scanned.analysis.messagingStrategy.currentState && (
                              <div className="mb-3">
                                <p className="text-xs font-semibold text-santa-teal/80 mb-1">Current State (Learn):</p>
                            ...
                            {scanned.analysis.messagingStrategy.transitionMessage && (
                              <div className="mb-3">
                                <p className="text-xs font-semibold text-santa-teal/80 mb-1">Transition Message (Ultra):</p>
                            ...
                            {scanned.analysis.messagingStrategy.tone && (
                              <div className="mb-3">
                                <p className="text-xs font-semibold text-santa-teal/80 mb-1">Recommended Tone:</p>
                            ...
                            {scanned.analysis.messagingStrategy.keyMessages && scanned.analysis.messagingStrategy.keyMessages.length > 0 && (
                              <div>
                                <p className="text-xs font-semibold text-santa-teal/80 mb-1">Key Messages:</p>
                        ...
                        {scanned.analysis.instructorRecommendations && (
                          <div className="mt-3 p-4 bg-yellow-50 border-2 border-yellow-300 rounded-lg">
                            ...
                            {scanned.analysis.instructorRecommendations.addDevShellLink && (
                              <div className="mb-3 p-2 bg-white rounded border border-yellow-200">
                                ...
                                {scanned.analysis.instructorRecommendations.devShellLinkText && (
                                  <p className="text-xs text-santa-teal/80 mt-1 italic">
                            ...
                            {scanned.analysis.instructorRecommendations.workshopMention && (
                              <div className="mb-3">
                                <p className="text-xs font-semibold text-santa-teal/80 mb-1">Workshop Mention:</p>
                            ...
                            {scanned.analysis.instructorRecommendations.ultraFeaturesToHighlight && scanned.analysis.instructorRecommendations.ultraFeaturesToHighlight.length > 0 && (
                              <div className="mb-3">
                                <p className="text-xs font-semibold text-santa-teal/80 mb-1">Ultra Features to Highlight:</p>
                                <ul className="list-disc list-inside text-xs text-gray-700">
                            ...
                            {scanned.analysis.instructorRecommendations.transitionGuidance && (
                              <div>
                                <p className="text-xs font-semibold text-santa-teal/80 mb-1">Transition Guidance:</p>
                                ...
                                <div key={idx} className="p-3 bg-white rounded border border-gray-200">
                                  ...
                                  {change.currentText && (
                                    <div className="mb-2 p-2 bg-gray-100 rounded">
                                      <p className="text-xs font-semibold text-santa-teal/80 mb-1">Current:</p>
                                      <p className="text-xs text-gray-700 line-through">{change.currentText}</p>
                                    </div>
                                  )}
                                  {change.newText && (
                                    <div className="mb-2 p-2 bg-green-50 rounded">
                                      <p className="text-xs font-semibold text-santa-teal/80 mb-1">New:</p>
                                      <p className="text-xs text-green-700">{change.newText}</p>
                                    </div>
                                  )}
                                  {change.reason && (
                                    <p className="text-xs text-santa-teal/80 italic">Why: {change.reason}</p>
                                  )}
                                </div>
                              ))}
                            </div>
                        ...
                        {scanned.analysis.documentStructure && (
                          <div className="mt-4 border-t border-gray-200 pt-4">
                            ...
                            {scanned.analysis.documentStructure.headingHierarchy.length > 0 && (
                              <div className="mb-3">
                                <p className="text-xs font-semibold text-santa-teal/80 mb-2">Heading Hierarchy:</p>
                            ...
                            {scanned.analysis.documentStructure.landmarks.length > 0 && (
                              <div>
                                <p className="text-xs font-semibold text-santa-teal/80 mb-2">Landmark Regions:</p>
                                <div className="flex gap-2 flex-wrap">
